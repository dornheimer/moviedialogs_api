...
@bp.route('/moviedb/api/characters/<string:character_id>', methods=['GET'])
def get_character(character_id):
    character = Character.query.get_or_404(character_id)
    return jsonify({'character': object_as_dict(character)})


@bp.route('/moviedb/api/conversations/<int:conversation_id>', methods=['GET'])
def get_conversation(conversation_id):
    conversation = Conversation.query.get_or_404(conversation_id)
    return jsonify({'conversation': object_as_dict(conversation)})


@bp.route('/moviedb/api/genres/<int:genre_id>', methods=['GET'])
def get_genre(genre_id):
    genre = Genre.query.get_or_404(genre_id)
    return jsonify({'genre': object_as_dict(genre)})


@bp.route('/moviedb/api/lines/<string:line_id>', methods=['GET'])
def get_line(line_id):
    line = Line.query.get_or_404(line_id)
    return jsonify({'line': object_as_dict(line)})


@bp.route('/moviedb/api/movies/<string:movie_id>', methods=['GET'])
...
@bp.route('/moviedb/api/movies', methods=['GET'])
def get_movies():
